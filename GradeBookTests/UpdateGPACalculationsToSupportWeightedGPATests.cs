...
namespace GradeBookTests
{
    public class UpdateGPACalculationsToSupportWeightedGPATests
    {
        ...
        public void GetWeightedGPATest()
        {
            ...
            Assert.False((double)method.Invoke(gradeBook, new object[] { 'A', StudentType.Honors }) == 4, "`GradeBook.GradeBooks.BaseGradeBook`'s `GetGPA` method weighted a student's grade when the gradebook was not weighted.");
            Assert.False((double)method.Invoke(gradeBook, new object[] { 'A', StudentType.DualEnrolled }) == 4, "`GradeBook.GradeBooks.BaseGradeBook`'s `GetGPA` method weighted a student's grade when the gradebook was not weighted.");
        }
    }
}
